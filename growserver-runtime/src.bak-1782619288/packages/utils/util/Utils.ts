...
export async function downloadMkcert() {
  ...
  // The download has no execute bit; mkcert -install fails with EACCES without
  // this (upstream Dockerfile even comments out its own chmod). chmod 0755 so
  // setupMkcert() can actually run the binary.
