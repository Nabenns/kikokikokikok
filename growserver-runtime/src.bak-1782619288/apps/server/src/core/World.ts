...
`,
  ...
  public async enter(peer: Peer, x: number, y: number) {
    ...
    // HEADER: uint16 version(2) + uint32 reserved(4) + uint16 name_len(2) + char[] name(N) + uint32 width(4) + uint32 height(4) + uint16 tile_count(2) = 18+N
    const HEADER_LENGTH = this.worldName.length + 18;
    ...
    // HEADER (per spec: uint16 version, uint32 reserved=0, uint16 name_len, char[] name, uint32 width, uint32 height, uint16 tile_count)
    buffer.writeUint16LE(0x14); // version = 20
    buffer.writeUint32LE(0x00, 2); // reserved = 0 (NOT 0x40)
    ...
    buffer.writeUint16LE(blockCount, 16 + this.worldName.length); // uint16, NOT uint32
    ...
    // 7-byte padding after header (per spec: uint8[7] reserved/padding)
    const unk1 = Buffer.alloc(7);
    unk1.fill(0);
    ...
    // 12-byte padding before dropped objects (per spec: uint8[12] reserved/padding)
    const unk2 = Buffer.alloc(12);

    // Drop data (per spec: uint32 last_object_uid, uint32 object_count, then per-object: uint16 item_id, float x, float y, uint16 count, uint32 object_id)
    ...
    dropData.writeUInt32LE(droppedUid); // last_object_uid
    dropData.writeUInt32LE(droppedItemsCount, 4); // object_count

    let pos = 8;
    this.data.dropped?.items?.forEach((item) => {
      dropData.writeUInt16LE(item.id, pos); // item_id
      dropData.writeFloatLE(item.x, pos + 2); // pos_x
      dropData.writeFloatLE(item.y, pos + 6); // pos_y
      dropData.writeUInt16LE(item.amount < -1 ? 0 : item.amount, pos + 10); // count (uint16, not uint8)
      dropData.writeUInt32LE(item.uid, pos + 12); // object_id

      pos += 16;
    });

    // Weather (per spec: uint16 base_weather_id, uint16 current_weather_id)
    const weatherData = Buffer.alloc(4);
    weatherData.writeUint16LE(this.data.weather.id); // base_weather_id
    weatherData.writeUint16LE(this.data.weather.id, 2); // current_weather_id (same as base for now)

    // Ownership (per spec: uint32 owner_user_id=0, uint32 world_flags=0, int32 main_lock_index=-1)
    const ownershipData = Buffer.alloc(12);
    ownershipData.writeUInt32LE(0, 0); // owner_user_id = 0 (no owner)
    ownershipData.writeUInt32LE(0, 4); // world_flags = 0
    ownershipData.writeInt32LE(-1, 8); // main_lock_index = -1 (no world lock)

    const worldMap = Buffer.concat([
      buffer,
      Buffer.concat([unk1, Buffer.from(blockBytes)]),
      Buffer.concat([unk2, dropData, weatherData, ownershipData]),
    ]);

    console.log(`[WORLD] worldMap total size: ${worldMap.length} bytes`);
    console.log(`[WORLD] header: ${buffer.length}, unk1: ${unk1.length}, blocks: ${blockBytes.length}, unk2: ${unk2.length}, drop: ${dropData.length}, weather: ${weatherData.length}, owner: ${ownershipData.length}`);
    console.log(`[WORLD] first 64 bytes hex: ${worldMap.slice(0, 64).toString("hex")}`);
