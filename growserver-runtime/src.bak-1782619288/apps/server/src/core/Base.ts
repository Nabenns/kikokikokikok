...
export class Base {
  ...
  public async getLatestCdn() {
    try {
      ...
      // The external CDN (mari-project.jad.li) is flaky / returns 500. If it
      // failed to give us a usable items.dat name, fall back to whatever was
      // already downloaded into the local dat cache during `setup`. Without
      // this, every transient CDN hiccup crash-loops the server even though the
      // items.dat it needs is sitting on disk.
      if (!data.itemsDatName) {
        const local = this.localItemsDatName();
        if (local) {
          logger.warn(`CDN unusable; falling back to local items.dat: ${local}`);
          data.itemsDatName = local;
        }
      }
      ...
      const local = this.localItemsDatName();
      if (local) {
        logger.warn(`CDN fetch threw; falling back to local items.dat: ${local}`);
        return { version: "", uri: "", itemsDatName: local };
      }
  ...
  /**
   * Newest items-v*.dat already present in the local dat cache (downloaded at
   * setup time). Returns "" if none exists. Used as a CDN-down fallback.
   */
  private localItemsDatName(): string {
    try {
      const datDir = join(__dirname, ".cache", "growtopia", "dat");
      if (!existsSync(datDir)) return "";
      const re = /^items-v(\d+\.\d+)\.dat$/;
      const cands = readdirSync(datDir)
        .filter((f) => re.test(f))
        .sort(
          (a, b) =>
            parseFloat(a.match(re)![1]) - parseFloat(b.match(re)![1]),
        );
