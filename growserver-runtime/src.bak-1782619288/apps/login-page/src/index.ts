  ...
  app.post("/player/growid/checktoken", async (ctx) => {
    try {
      ...
      if (contentType.includes("application/json")) {
        ...
        // Fallback: read raw body as text (client often sends no content-type)
        const raw = await ctx.req.text();
        // Could be "refreshToken=eyJ..." or just "eyJ..." or JSON
        if (raw.startsWith("{")) {
          try {
        ...
        } else if (raw.includes("refreshToken=")) {
          const params = new URLSearchParams(raw);
          refreshToken = params.get("refreshToken") || undefined;
        } else if (raw.trim()) {
          refreshToken = raw.trim();
        }
      }

      // URL-decode the token (client sends JWT with %2E for dots, %2D for dashes, %5F for underscores)
      if (refreshToken) {
