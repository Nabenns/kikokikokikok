...
public:
    ...
    */
    ...
    u_short fires_removed{}; // @note also used for achievements
    u_short gbc_pity{}; // @note GBC pity; for each 100 will receive super GBC

    /* gameplay fields */
    std::string title{}; // @note equipped title
    std::time_t last_daily{}; // @note last daily bonus claim time
    int guild_id{}; // @note guild membership
    std::string friends_csv{}; // @note friends list CSV: name:ignore;name:ignore;...
    short pet_id{}; // @note equipped pet item id
    std::string pet_name{}; // @note pet name
    short pet_level{}; // @note pet level
    int pet_xp{}; // @note pet xp
    std::string quests{}; // @note daily quests CSV: quest_id:progress:completed;...
    std::string stats{}; // @note stats CSV
};
