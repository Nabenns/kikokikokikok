...
void peer::mysql_select_all()
{
    ...
    this->role       = this->mysql_select<unsigned>("role");
    this->skin_color = this->mysql_select<unsigned>("skin_color");
    this->hair_color = this->mysql_select<signed>("hair_color");
    this->slot_size  = this->mysql_select<signed>("slot_size");
    this->banned     = this->mysql_select<signed>("banned");
    this->muted      = this->mysql_select<signed>("muted");
    this->last_daily = this->mysql_select<std::time_t>("last_daily", "UNIX_TIMESTAMP");
    this->guild_id   = this->mysql_select<signed>("guild_id");
    this->pet_id     = this->mysql_select<signed>("pet_id");
    this->pet_level  = this->mysql_select<signed>("pet_level");
    this->pet_xp     = this->mysql_select<signed>("pet_xp");

    /* trim null bytes from NULL column values (make_bind_out pads to 1024) */
    auto trim_null = [](std::string &s) {
        if (auto pos = s.find('\0'); pos != std::string::npos)
            s.resize(pos);
    };

    this->title      = this->mysql_select<std::string>("title");
    trim_null(this->title);
    this->pet_name   = this->mysql_select<std::string>("pet_name");
    trim_null(this->pet_name);
    this->quests     = this->mysql_select<std::string>("quests");
    trim_null(this->quests);
    this->stats      = this->mysql_select<std::string>("stats");
    trim_null(this->stats);
    this->friends_csv = this->mysql_select<std::string>("friends");
    trim_null(this->friends_csv);
    load_friends(this);

    /* load clothing (CSV: val,val,val,...) */
    std::string cloth_str = this->mysql_select<std::string>("clothing");
    trim_null(cloth_str);
    if (!cloth_str.empty())
    {
        std::vector<std::string> parts = readch(cloth_str, ',');
        for (std::size_t i = 0; i < parts.size() && i < this->clothing.size(); ++i)
            this->clothing[i] = std::stof(parts[i]);
    }

    /* load inventory (CSV: id:count,id:count,...) */
    std::string inv_str = this->mysql_select<std::string>("inventory");
    trim_null(inv_str);
    if (!inv_str.empty())
    {
        this->slots.clear();
        std::vector<std::string> items = readch(inv_str, ',');
        for (const auto& item : items)
        {
            std::vector<std::string> kv = readch(item, ':');
            if (kv.size() >= 2)
                this->slots.emplace_back((short)std::stoi(kv[0]), (short)std::stoi(kv[1]));
        }
    }
}
...
{
    if (this->growid.empty()) return; // @note safety: don't save unnamed peers

    this->mysql_update<signed>("gems", this->gems);
    this->mysql_update<unsigned>("level", this->level[0]);
    this->mysql_update<unsigned>("xp", this->level[1]);
    this->mysql_update<unsigned>("role", this->role);
    ...
    this->mysql_update<std::string>("title", this->title);
    this->mysql_update<signed>("guild_id", this->guild_id);
    this->mysql_update<signed>("pet_id", this->pet_id);
    this->mysql_update<std::string>("pet_name", this->pet_name);
    this->mysql_update<signed>("pet_level", this->pet_level);
    this->mysql_update<signed>("pet_xp", this->pet_xp);
    this->mysql_update<std::string>("quests", this->quests);
    this->mysql_update<std::string>("stats", this->stats);
    this->mysql_update<std::string>("friends", this->friends_csv);

    /* serialize clothing → CSV */
    std::string cloth_str;
    for (std::size_t i = 0; i < this->clothing.size(); ++i)
    {
        if (i > 0) cloth_str += ",";
        cloth_str += std::format("{}", this->clothing[i]);
    }
    this->mysql_update<std::string>("clothing", cloth_str);

    /* serialize inventory → CSV (id:count,id:count,...) */
    std::string inv_str;
    for (std::size_t i = 0; i < this->slots.size(); ++i)
    {
        if (i > 0) inv_str += ",";
        inv_str += std::format("{}:{}", this->slots[i].id, this->slots[i].count);
    }
    this->mysql_update<std::string>("inventory", inv_str);
}
