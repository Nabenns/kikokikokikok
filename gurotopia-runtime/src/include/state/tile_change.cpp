...
#include "action/storage_box.hpp"
#include "action/bulletin_board.hpp"
#include "action/fishing.hpp"
#include "action/cooking_oven.hpp"
#include "action/chemical_combiner.hpp"
#include "action/safe_vault.hpp"
#include "action/geiger_charger.hpp"
#include "action/magplant.hpp"
#include "item_activate_object.hpp"
#include "https/metrics.hpp"
    ...
    try
    {
        ...
        if (state.id == 18) // @note punching a block
        {
            ...
            switch (item->type)
            {
                ...
                {
                    // @note Ghost Block — toggle visible/invisible when punched
                    if (item->id == 5814/*GHOST_BLOCK*/)
                    {
                        ...
                        on::ConsoleMessage(event.peer, (block.state[2] & S_TOGGLE) ? 
                            "`oThe Ghost Block is now `2visible``." : "`oThe Ghost Block is now `5invisible``.");
                        return; // @note don't break the block
                    }
                    block.state[2] ^= S_TOGGLE;
                ...
                case type::CHEST:
                {
                    // @note Treasure Chest loot — drop random gems and rare items before toggling
                    if (!(block.state[2] & S_TOGGLE)) // @note only drop loot when opening (not closing)
                    {
                        u_char gems = ransuu[{5, 50}];
                        for (short i : {10, 5, 1}/* gem type */)
                            for (; gems >= i; gems -= i/* downgrade type */)
                                add_drop(event, {112, i}, state.punch.by_32());

                        // @note rare item drop (20% chance)
                        if (!ransuu[{0, 4}])
                        {
                            short reward =
                                (!ransuu[{0, 9}]) ? 1458/*GHC*/ :
                                (!ransuu[{0, 7}]) ? 362/*Angel Wings*/ :
                                (!ransuu[{0, 5}]) ? 1470/*Ruby Necklace*/ :
                                (!ransuu[{0, 4}]) ? 2384/*Love Bug*/ :
                                (!ransuu[{0, 3}]) ? 3388/*Heartbreaker Hammer*/ :
                                388/*Perfume*/;
                            add_drop(event, ::slot(reward, 1), state.punch.by_32());

                            std::string message = std::format("msg|`2{} found a `w{}`` in a Treasure Chest!", pPeer->growid,
                                std::ranges::find(items, reward, &::item::id)->raw_name);
                ...
                case type::PINATA:
                {
                    // @note Pinata — spawn candy drops when punched
                    if (!(block.state[2] & S_TOGGLE)) // @note only on first hit
                    {
                        // @note candy drops — random items + gems
                        u_char gems = ransuu[{10, 100}];
                        for (short i : {100, 50, 10, 5, 1}/* gem type */)
                            for (; gems >= i; gems -= i/* downgrade type */)
                                add_drop(event, {112, i}, state.punch.by_32());

                        // @note drop random candy items (3-5 drops)
                        u_char drops = ransuu[{3, 5}];
                        for (u_char d = 0; d < drops; ++d)
                        {
                            short candy =
                                (!ransuu[{0, 3}]) ? 3404/*Sour Lollipop*/ :
                                (!ransuu[{0, 3}]) ? 3406/*Sweet Lollipop*/ :
                                (!ransuu[{0, 4}]) ? 758/*Roulette Wheel*/ :
                                (!ransuu[{0, 4}]) ? 3458/*Raw Fish*/ :
                                (!ransuu[{0, 5}]) ? 2766/*Wheat*/ :
                                3462/*Raw Meat*/;
                            add_drop(event, ::slot(candy, ransuu[{1, 3}]), state.punch.by_32());
                        }

                        std::string message = std::format("msg|`o{} `2broke a Pinata`` and candy went everywhere!", pPeer->growid);
                        peers(pPeer->recent_worlds.back(), PEER_SAME_WORLD, [&event, &pPeer, message](ENetPeer &p)
                        {
                ...
                case type::STORM_CLOUD:
                {
                    // @note Storm Cloud — toggle weather like weather machine
                    ::block &weather_machine = world->blocks[cord(world->现weather.x, world->现weather.y)];
        ...
        else if (state.id == 32)
        {
            switch (item->type)
            {
                ...
                case type::BULLETIN:
                {
                    // @note Bulletin Board — reuse sign edit pattern for public messages
                    send_varlist(event.peer, {
                        "OnDialogRequest",
                        std::format(
                            "set_default_color|`o\n"
                ...
                case type::STORAGE_BOX:
                {
                    // @note Storage Box — show deposit/withdraw dialog
                    action::storage_box(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
                case type::FISHING_BLOCK:
                {
                    // @note Fishing Block — start fishing mini-game
                    action::fishing(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
                case type::COOKING_OVEN:
                {
                    // @note Cooking Oven — show recipe combine dialog
                    action::cooking_oven(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
                case type::CHEMICAL_COMBINER:
                {
                    // @note Chemical Combiner — show combine dialog
                    action::chemical_combiner(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
                case type::SAFE_VAULT:
                {
                    // @note Safe Vault — password-protected storage
                    action::safe_vault(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
                case type::GEIGER_CHARGER:
                {
                    // @note Geiger Charger — radioactive item generation
                    action::geiger_charger(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
                case type::MAGPLANT:
                {
                    // @note MAGPLANT — auto-farm storage
                    action::magplant(event, std::format("tilex|{}|tiley|{}", state.punch.x, state.punch.y));
                    break;
                }
            }
